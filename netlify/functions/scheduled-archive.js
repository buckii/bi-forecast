...
async function archiveQuickBooksData(calculator, startOffset, monthsCount) {
  ...
  try {
    ...
    console.log(
      `[Archive QB] ✓ Captured ${invoices.length} invoices (${openInvoices.length} open), ${journalEntries.length} journal entries, ${delayedCharges.length} delayed charges`,
    )
...
async function archivePipedriveData(calculator) {
  ...
  try {
    ...
    console.log(
      `[Archive PD] ✓ Captured ${openDeals.length} open deals, ${wonUnscheduled.length} won unscheduled deals`,
    )
