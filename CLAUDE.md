...
**Shared formulas (`src/lib/metrics-formulas.js`)**: Revenue/metrics math (current/3-month/year forecast, unbilled, cash, days-cash) lives in one pure ESM module so the Pinia store, the Dashboard, and the Netlify functions all produce identical numbers. It's ESM (named exports + default): the browser imports it directly via Vite; CommonJS functions load it with a cached dynamic `import()` (e.g. `metrics-plain.cjs` — note the `.cjs` extension is required so the function loads as CommonJS under the project's `"type": "module"`; `netlify.toml` `included_files` ships `src/lib` with it). Change a forecast definition here, not in a consumer.
