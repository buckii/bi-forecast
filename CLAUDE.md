...
**Shared formulas (`src/lib/metrics-formulas.js`)**: Revenue/metrics math (current/3-month/year forecast, unbilled, cash, days-cash) lives in one pure ESM module so the Pinia store and the Dashboard produce identical numbers. It's ESM (named exports + default) and imported directly by the Vite frontend.
