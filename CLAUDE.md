...
**Shared formulas (`src/lib/metrics-formulas.js`)**: Revenue/metrics math (current/3-month/year forecast, unbilled, cash, days-cash) lives in one pure ESM module so the Pinia store, the Dashboard, and the Netlify functions all produce identical numbers. It's ESM (named exports + default): the browser imports it directly via Vite; CommonJS functions load it with a cached dynamic `import()` (e.g. `metrics-plain.js`). Change a forecast definition here, not in a consumer.
