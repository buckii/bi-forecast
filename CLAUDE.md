...
**Shared formulas (`src/lib/metrics-formulas.js`)**: Revenue/metrics math (current/3-month/year forecast, unbilled, cash, days-cash, days-of-work) lives in one pure ESM module so the Pinia store and the Dashboard produce identical numbers. It's ESM (named exports + default) and imported directly by the Vite frontend.

**Days of Work** (`daysOfWork`/`allDaysOfWork`): the horizon (in days from the as-of date) at which cumulative booked revenue can no longer sustain a given net margin — "how long until we run out of work if we win nothing more?". Walks forward month-by-month, accruing revenue against `monthlyExpenses`/month, and tracks the **cumulative surplus** (`revenue - k * expenses` per month, `k = 1/(1 - targetMargin)`; break-even → `targetMargin = 0`). The horizon is the **last** point at which that surplus is still ≥ 0 — the point beyond which cumulative revenue can never catch up — interpolated within the crossing month. Taking the last crossing rather than the first is deliberate: it spreads a strong month's revenue back over a lean one, so a single under-target month (typically the current one, already partly invoiced) does not collapse the horizon to ~0. Four variations: target-margin (uses `targetNetMargin` setting) vs break-even, and forecasted (incl. `weightedSales`) vs won (committed only). By this math the target-margin horizon is **shorter** than break-even (a higher margin needs more revenue). Returns `null` when expenses ≤ 0 / no data, `0` when nothing in the window can cover the current shortfall, and a window-length floor when the threshold is never crossed within available months.

**Do NOT import this module from a Netlify function.** A function reaching across the tree into ESM frontend code breaks the function bundler (the file ships as raw source and either fails to load under `"type": "module"`, or the bundler rewrites `import()` to `require()` and chokes on `export`). `metrics-plain.cjs` is therefore intentionally self-contained CommonJS with its own copy of the formulas it needs — keep that copy in lockstep with `metrics-formulas.js` so the plain-text metrics match the dashboard. (`.cjs` is also required so the function loads as CommonJS under the project's `"type": "module"`.)
