...
**Days of Work** (`daysOfWork`/`allDaysOfWork`): the horizon (in days from the as-of date) at which cumulative booked revenue can no longer sustain a given net margin — "how long until we run out of work if we win nothing more?". Walks forward month-by-month, accruing revenue against `monthlyExpenses`/month, and interpolates the crossing day where cumulative `revenue/expenses` hits the threshold (`k = 1/(1 - targetMargin)`; break-even → `targetMargin = 0`). Four variations: target-margin (uses `targetNetMargin` setting) vs break-even, and forecasted (incl. `weightedSales`) vs won (committed only). By this math the target-margin horizon is **shorter** than break-even (a higher margin needs more revenue). Returns `null` when expenses ≤ 0 / no data, and a window-length floor when the threshold is never crossed within available months.
